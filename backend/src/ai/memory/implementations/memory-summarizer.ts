...
export class MemorySummarizer implements AiMemorySummarizer {
  private llm = new OllamaProvider('llama3.1');
  ...
  async summarize(kind: AiMemoryKind, content: unknown): Promise<AiMemorySummary> {
    try {
      ...
      const response = await this.llm.generateStructured(
        [
          new SystemMessage(`You are an AI Memory Compression Engine. Compress the provided ${kind} context into a dense summary. Retain all names, IDs, dates, and strict constraints. Drop conversational filler.`),
          new HumanMessage(contentStr)
        ],
        zodToJsonSchema(SummarySchema)
      );
